...
用户自己的排版工具决定。著者与年份括号之间保留 APA 生成的一个空格。空格
过滤器保留叙述式引用前的源空格和引用内部间隔，只删除引用后、继续中文正文前
的源空格。
...
Pandoc citeproc 并本地化连接符；两个过滤器的顺序不能颠倒。连接符过滤器只把
叙述式引用中连接两位个人作者的独立 `&` 改为“和”，保留其左右已有的空格；
机构作者名称、夹注引用和参考文献表不受影响。
